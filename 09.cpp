...
int main()
{
    ...
    while (getline(cin, input)) {
        ...
        if (words.size() != 3) isValid = false;
        
        // Check if the words are in proper sequence;
        else {
            if (find(subject.begin(), subject.end(), words[0]) == subject.end()) isValid = false;
            if (find(verb.begin(), verb.end(), words[1]) == verb.end()) isValid = false;
            if (find(noun.begin(), noun.end(), words[2]) == noun.end()) isValid = false;
        }
